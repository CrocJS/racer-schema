...
Schema.prototype.setupStore = function(store) {
  ...
  store.shareClient.use('submit', function(shareRequest, done) {
    ...
    if (opData.create) {
      ...
      var oldValidate = opData.validate;
      opData.validate = function(opData, data) {
        var oldValidateResult = oldValidate && oldValidate.apply(this, arguments);
        var doc = data.data;
        // Array has only one op here, that`s how ShareJS applies ops
        var op = opData.op[0];
        var paths = op.p;

        return oldValidateResult || self.validate(doc, rootSchema, paths, op.contexts);
      }
